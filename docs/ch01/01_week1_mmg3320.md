...
## What is a Shell? 
A shell is a program that allows users to type commands to interact with the operating system. With the shell, you can execute both complex tasks, such as running modeling software and simple commands, like creating an empty directory with only one line of code. 

The most popular Unix shell is Bash (the Bourne Again SHell) — so named because it is derived from a shell written by Stephen Bourne. Learning to use the shell requires time and effort. While a GUI presents you with choices to select, CLI does not automatically display these to you. Instead, you will need to learn specific commands. This will resemble learning a new language! 
...
![compute cluster](../img/compute_cluster.png)
...
![bluemoon cluster](../img/bluemoon-logo.png)
...
![RStudio-Logo](../img/RStudio-Logo.png)
