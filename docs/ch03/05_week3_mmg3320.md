...
* **`sort` is a command is used to sort lines in text files in a particular order.** The sort command comes with many options, some of them are further explained below: 
...
!!! example "Class Exercise #3" 
	...
	**NOTE: It is never my intention to rush you. If you do find you need more time to participate, please let me know.**
	...
	+ First line: You will write this line on your own. This line should: 
	...
	+ Second line: You will copy and paste the `echo` line below: 
    	
		+ You are adding some verbosity to your script by using the `echo` command. The `echo` command is used to display a line of text that is passed in as an argument. This is a bash command that is used to output status to the screen or file. 
	...
	+ Third line: You will write this line on your own. This line should:
