...
1. After downloading the FASTQC files from GEO. This will contain an aggregated report from FASTQC only. 
2. Then again after performing alignment. This will contain results from FASTQC & HISAT2 or STAR (depending on the aligner selected). 
...
Today, you will be working on this exercise for the last 20 minutes of class. Your objective will be to: 
...
1. Download the data. These data are in the form of a `.zip` file output from FASTQC. These files are located on the shared drive: 
    ...
    ```bash
    /gpfs1/cl/mmg3320/course_materials/multiqc_exercise
    ```
...
3. View and interpret the multiqc results for each project using the metrics below. There is a worksheet to be filled out and turned into Brightspace. This worksheet is posted on Brightspace. 
